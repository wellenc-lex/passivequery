...
###Checks if ip is Cloudflare / stormwall / etc ip range
def check_subnet(ip):
    flag = 0
    networks = ["103.21.244.0/22", "103.22.200.0/22", "103.31.4.0/22", "104.16.0.0/12", "104.24.0.0/14", "108.162.192.0/18", "131.0.72.0/22",
            ...
            "23.74.0.0/23", "120.52.22.96/27", "205.251.249.0/24", "180.163.57.128/26", "204.246.168.0/22", "205.251.252.0/23", "54.192.0.0/16", "204.246.173.0/24", "54.230.200.0/21", 
            ...
            "52.52.191.128/26", "34.216.51.0/25", "34.223.12.224/27", "34.223.80.192/26", "35.162.63.192/26", "35.167.191.128/26", "44.227.178.0/24", "44.234.108.128/25", "44.234.90.252/30"]
...
def export_results (querys, output, shodan, censys, spyse, binaryedge):

    valid = []

    try:
        print ("\nExporting the results as txt")
        results = shodan+spyse+censys+binaryedge
        unique=set(results)
        print("output:")
        print(unique)

        for ip in unique:
            flag = check_subnet(ip)
            if flag == 0:
                valid.append(ip)
